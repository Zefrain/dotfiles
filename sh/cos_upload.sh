...
usage() {
  cat <<'USAGE'
Usage: cos_upload.sh <src_dir> <dest_cos_dir>

Upload local files to COS with the current coscmd config.

Arguments:
  src_dir       Local source directory.
  dest_cos_dir  COS destination directory, for example checkpoints/.
...
Examples:
  cos_upload.sh /data checkpoints/
  COS_EXTRA_IGNORE='*.log,cache/*' cos_upload.sh . checkpoints/
USAGE
}
...
main() {
  ...
  coscmd upload -r -s --ignore "$ignore_rules" "$src_dir" "$dest_cos_dir"
}
